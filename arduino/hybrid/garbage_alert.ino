...
#define maxBlockedTime 3000 // Blocked time max counter value
...
void setup() {
  ...
  Blynk.begin(auth, WIFISSID, PASSWORD); // Setting Blynk connection
...
void loop() {
  ...
  if (pinRead0 < 500) {
    ...
    if (counter == maxBlockedTime) {
      Blynk.notify("POUBELLE2 IS FULL"); //alert for blink notification
